...
function OTDepartment() {
    ...
    try {
      ...
      const [deptsData, orphanData] = await Promise.all([
        ...
        apiFetch(`${API_BASE}/ot-custom-lists-all?_t=${Date.now()}`).catch(err => {
          console.warn('⚠️ Orphan lists fetch failed:', err.message);
          return { success: false, orphanLists: [] };
        })
      ]);
  ...
  const handleDeleteDept = async (id, name) => {
    ...
    try {
      const check = await apiFetch(
        `${API_BASE}/ot-custom-lists?deptCode=${encodeURIComponent(id)}&_t=${Date.now()}`
      );
      listCount = check.data?.length || 0;
    } catch (err) {
      console.warn('Could not check list count:', err.message);
    }

    let confirmMsg;
    if (listCount > 0) {
      confirmMsg =
        `⚠️ تحذير: القسم "${name}" يحتوي على ${listCount} لستة!\n\n` +
        `حذف القسم سيؤدي إلى حذف جميع اللستات والمعدات المرتبطة به.\n\n` +
        `هل أنت متأكد؟`;
    } else {
      confirmMsg = `Delete department "${name}"?`;
    }
  ...
  // ============================================================
  // ✅ ORPHAN LISTS — إعادة الربط
  // ============================================================
  const handleReassignOrphan = async (listId, targetDeptCode) => {
    if (!targetDeptCode) {
    ...
    try {
      setSaving(true);
      const data = await apiFetch(`${API_BASE}/ot-move-list-to-dept`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ listId, targetDeptCode })
      });
      ...
      console.error("❌ handleReassignOrphan:", err);
      alert("❌ فشل نقل اللستة: " + err.message);
    } finally {
      setSaving(false);
    }
  };

  const handleFixAllOrphans = async (targetDeptCode) => {
    if (!targetDeptCode) {
      alert("الرجاء اختيار قسم أولاً");
      return;
    }

    const count = orphanLists.length;
    if (!window.confirm(`نقل جميع اللستات اليتيمة (${count}) إلى هذا القسم؟`)) return;

    try {
      setSaving(true);
      const data = await apiFetch(`${API_BASE}/ot-fix-orphan-lists`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ targetDeptCode })
      });

      if (data.success) {
        alert(`✅ تم نقل ${data.fixed} لستة بنجاح`);
        await loadDepartments();
      }
    } catch (err) {
      console.error("❌ handleFixAllOrphans:", err);
      alert("❌ فشل نقل اللستات: " + err.message);
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div style={{
      ...
      {/* ===== ORPHAN LISTS WARNING ===== */}
      {orphanLists.length > 0 && (
        <div style={{
          background: "#fef3c7",
          ...
          <div style={{
            ...
            flexWrap: "wrap",
            gap: "10px",
            marginBottom: orphanSectionOpen ? "12px" : 0
          }}>
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              ...
              <div>
                <h3 style={{
                  ...
                  {orphanLists.length} لستة غير مرتبطة بقسم
                </h3>
                <p style={{
                  fontSize: isMobile ? "11px" : "12px",
                  color: "#78350f",
                  ...
                  هذه اللستات موجودة في قاعدة البيانات لكن قسمها انحذف. اضغط لإعادة ربطها.
                </p>
              </div>
            </div>
            <button
              onClick={() => setOrphanSectionOpen(!orphanSectionOpen)}
              style={{
                padding: "6px 14px",
                background: "#f59e0b",
                color: "white",
            ...
            <>
              {isAdmin && departments.length > 0 && (
                <div style={{
                  ...
                  <span style={{ fontSize: isMobile ? "12px" : "13px", fontWeight: "600", color: "#92400e" }}>
                    نقل الكل إلى:
                  </span>
                  <select
                    id="bulk-orphan-dept"
                  ...
                  <button
                    onClick={() => {
                      const sel = document.getElementById('bulk-orphan-dept');
                      handleFixAllOrphans(sel.value);
                    }}
                    disabled={saving}
                    style={{
                      padding: "6px 14px",
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{
                        ...
                        ID: {list.id} • قسم قديم: {list.deptCode}
                        {list.equipmentCount !== undefined && ` • ${list.equipmentCount} معدات`}
                      </div>
                    </div>
                    ...
                    {isAdmin && departments.length > 0 && (
                      <div style={{
                        ...
                        <button
                          onClick={() => {
                            const sel = document.getElementById(`orphan-dept-${list.id}`);
                            handleReassignOrphan(list.id, sel.value);
                          }}
                          disabled={saving}
                          style={{
                            padding: "5px 10px",
