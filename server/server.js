...
try {
  ...
  console.warn(" AI Search routes not loaded:", e.message);
}
  ...
  try {
    ...
    if (!targetDept) {
      ...
      return res.status(404).json({
        ...
        availableDepts: depts.map(d => ({ id: d.id, name: d.name })),
      });
    }
    ...
    console.error('❌ /api/ot-fix-orphan-lists error:', err);
    res.status(500).json({ success: false, message: err.message });
  }
});
...
app.post('/api/ot-move-list-to-dept', async (req, res) => {
  try {
    ...
    console.error('❌ /api/ot-move-list-to-dept error:', err);
    res.status(500).json({ success: false, message: err.message });
  }
});
...
app.post('/api/ot-custom-lists', async (req, res) => {
  try {
    ...
    if (existing) {
      console.warn(` List ${id} exists — updating`);
    ...
    console.log(` Custom list created: ${name} (id=${id}, deptCode=${newList.deptCode})`);
